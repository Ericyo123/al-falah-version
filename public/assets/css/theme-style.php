	...
	if( in_array( $key, $allowed ) ){
		if( is_array($val) ){
			foreach( $val as $val_key=>$val_val ){
				if( !empty($val_val) ){
				?>
	--pbmit-xhyre-<?php echo esc_attr($key); ?>-<?php echo esc_attr($val_key); ?>: <?php echo esc_attr($val_val); ?>;
<?php
				}
			}
		} else {
		?>
	--pbmit-xhyre-<?php echo esc_attr($key); ?>: <?php echo esc_attr($val) ?>;
<?php
		}
	}
}
...
foreach( $new_all_data as $key=>$val ){
	if( in_array( $key, $colors ) ){
		...
		?>
	--pbmit-xhyre-<?php echo esc_attr($key); ?>-rgb: <?php echo esc_attr( pbmit_hex2rgb_code($val) ) ?>;
<?php
	}
}
?>
...
if( $footer_column=='custom' ) :
	...
	@media screen and (min-width: 992px) {
		...
		.site-footer .pbmit-footer-widget.pbmit-footer-widget-col-1{
			-ms-flex: 0 0 var(--pbmit-xhyre-footer-1-col-width);
			flex: 0 0 var(--pbmit-xhyre-footer-1-col-width);
			max-width: var(--pbmit-xhyre-footer-1-col-width);
		}
		<?php endif; ?>
		<?php if( !empty($footer_column_2) && $footer_column_2!='hide' ) : ?>
		.site-footer .pbmit-footer-widget.pbmit-footer-widget-col-2{
			-ms-flex: 0 0 var(--pbmit-xhyre-footer-2-col-width);
			flex: 0 0 var(--pbmit-xhyre-footer-2-col-width);
			max-width: var(--pbmit-xhyre-footer-2-col-width);
		}
		<?php endif; ?>
		<?php if( !empty($footer_column_3) && $footer_column_3!='hide' ) : ?>
		.site-footer .pbmit-footer-widget.pbmit-footer-widget-col-3{
			-ms-flex: 0 0 var(--pbmit-xhyre-footer-3-col-width);
			flex: 0 0 var(--pbmit-xhyre-footer-3-col-width);
			max-width: var(--pbmit-xhyre-footer-3-col-width);
		}
		<?php endif; ?>
		<?php if( !empty($footer_column_4) && $footer_column_4!='hide' ) : ?>
		.site-footer .pbmit-footer-widget.pbmit-footer-widget-col-4{
			-ms-flex: 0 0 var(--pbmit-xhyre-footer-4-col-width);
			flex: 0 0 var(--pbmit-xhyre-footer-4-col-width);
			max-width: var(--pbmit-xhyre-footer-4-col-width);
		}
		<?php endif; ?>
	}
<?php endif; ?>
