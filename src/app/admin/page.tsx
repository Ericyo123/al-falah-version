              ...
              <div className={styles.emptyState}>
                ...
                <div style={{ marginTop: "24px" }}>
                  <button 
                    ...
                    onClick={async () => {
                      try {
                        const res = await fetch("/api/jobs/seed", { method: "POST", headers: { Authorization: `Bearer ${token}` } });
                        if (res.ok) {
                          showToast("Database seeded with demo jobs!", "success");
                          fetchJobs();
                        } else {
                          const err = await res.json();
                          showToast(err.error || "Failed to seed", "error");
                        }
                      } catch {
                        showToast("Network error", "error");
                      }
